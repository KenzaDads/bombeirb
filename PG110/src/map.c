...
struct map* map_new(int width, int height)
{
	...
	/*int level=0;
	char map_level[50] = "./data/map_";
	char niveau[10]="";
	char* chaine_separate;
	sprintf(niveau, "%d", level);
	strcat(map_level, niveau);
	FILE* fichier_map= NULL;
	fichier_map = fopen(map_level, "r");

	int TAILLE_MAX = height * (4 * width);
	char chaine[TAILLE_MAX];
	for (int k=0; k < height; k++){
		fgets(chaine, TAILLE_MAX, fichier_map);
		chaine_separate=strtok(chaine," ");
		for (int n=0; n < width; n++){
			map_set_cell_type(map, n, k, atoi(chaine_separate));
			chaine_separate=strtok(chaine," ");
		}
	}
	fclose(fichier_map);*/
...
{
	assert(map);  // Pq pas assert(map != 	NULL)
	return map->height;
}
